...
import { Badge, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, InputAdornment } from '@mui/material'
...
const ImageGrid = ({ images, onRemoveImage, primaryImageId, onSetPrimary }) => (
    <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4'>
        {images.map((image, index) => {
            const isPrimary = primaryImageId ? image.id === primaryImageId : index === 0;
            return (
                <div key={image.id} className='relative group'>
                    ...
                    />
                    {isPrimary && <Badge className='absolute top-2 left-2 bg-emerald-500 text-white'>Primary</Badge>}
                    <div className='absolute bottom-2 left-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity'>
                        <Button
                            ...
                            className={cn(
                                'flex-1 text-xs',
                                isPrimary ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'text-foreground'
                            )}
...
const FeatureItem = ({ feature, onEdit, onRemove, isEditing }) => (
    <div className={`flex items-center justify-between p-3 rounded-lg border transition-all ${isEditing
        ...
        <div className='flex gap-1'>
            <Button size='small' variant='outline' onClick={() => onEdit(feature)} className='h-8 w-8 p-0'>
...
const SpecItem = ({ spec, onEdit, onRemove, isEditing }) => (
    <div className={`flex items-center justify-between p-3 rounded-lg border transition-all ${isEditing
            ? 'bg-blue-500/10 border-blue-500/50 shadow-md'
            : 'bg-muted/20 border-border'
        }`}>
        ...
        <div className='flex gap-1'>
            <Button size='small' variant='outline' onClick={() => onEdit(spec)} className='h-8 w-8 p-0'>
                <Edit className='w-3 h-3' />
            </Button>
...
function ProductEditFields({ productId, product, categories }) {
    ...
    // Update errors when page errors change
    useEffect(() => {
    ...
    useEffect(() => {
        if (product) {
            setFormData({
                ...
                technical_specs: (product.technical_specs || []).map(spec => ({
            ...
            if (product.images) {
                ...
                // Set the primary image ID from existing data
                const primaryImg = loadedImages.find(img => img.is_primary)
                if (primaryImg) {
    ...
    const addFeature = useCallback(() => {
        ...
        if (editingFeatureId) {
            ...
            setFormData(prev => ({
                ...prev,
                features: prev.features.map(f => f.id === editingFeatureId ? { ...featureForm, id: editingFeatureId } : f)
            }))
    ...
    }, [featureForm, setFormData, setFeatureForm, editingFeatureId])
    ...
    const addTechnicalSpec = useCallback(() => {
        ...
        if (editingSpecId) {
            ...
            setFormData(prev => ({
                ...prev,
                technical_specs: prev.technical_specs.map(s => s.id === editingSpecId ? { ...specForm, id: editingSpecId } : s)
            }))
    ...
    }, [specForm, setFormData, setSpecForm, editingSpecId])
    ...
    // Event handlers
    const handleSetPrimaryImage = useCallback((imageId) => {
                        ...
                        <div className='lg:col-span-2 space-y-6'>
                                ...
                                <div className='space-y-4'>
                                    ...
                                    {/* {editingFeatureId !== null && (
                                        <div className='bg-blue-500/10 border border-blue-500/30 rounded-lg p-3 text-sm text-blue-400'>
                                            <strong>Editing feature:</strong> Click "Update Feature" below to save your changes, or "Cancel" to discard.
                                        </div>
                                    )} */}
                                    <div className='flex gap-2'>
                                        {/* Add Feature Button - Only shown when NOT editing */}
                                        {editingFeatureId === null && (
                                            <Button
                                                variant='outlined'
                                                onClick={addFeature}
                                        ...
                                        {/* Update Feature Button - Only shown when editing */}
                                        {editingFeatureId !== null && (
                                            <>
                                                <Button
                                                    variant='outlined'
                            ...
                            <div className='bg-card border border-border rounded-xl p-6 space-y-4'>
                                ...
                                <div className='space-y-4'>
                                    ...
                                    <div className='flex gap-2'>
                                        {/* Add Specification Button - Only shown when NOT editing */}
                                        {editingSpecId === null && (
                                            <Button
                                                variant='outlined'
                                        ...
                                        {/* Update Specification Button - Only shown when editing */}
                                        {editingSpecId !== null && (
                                            <>
                                                <Button
                                                    variant='outlined'
                            ...
                            <div className='bg-card border border-border rounded-xl p-6 space-y-4'>
                                ...
                                {images.length > 0 && <ImageGrid images={images} onRemoveImage={removeImage} primaryImageId={primaryImageId} onSetPrimary={handleSetPrimaryImage} />}
                            </div>
                                                            ...
                                                            <InputAdornment position='end'>
                                                                <div className='relative'>
                                                                    <input
                                                                        type='color'
                                                                        value={(() => {
                                                                            // * Extract hex color from variantForm.color if it's a hex value
                                                                            if (variantForm.color && variantForm.color.startsWith('#')) {
                                                                                return variantForm.color.length === 7 ? variantForm.color : '#000000'
                                                                            }
                                                                            return '#000000'
                                                                        })()}
                                                                        onChange={e => setVariantForm(prev => ({ ...prev, color: e.target.value }))}
                                                                        className='w-5 h-5 rounded cursor-pointer appearance-none !border-none !outline-none !p-0'
                                                                        style={{
                                                                            backgroundColor: variantForm.color && variantForm.color.startsWith('#')
                                                                                ? variantForm.color
                                                                                : '#000000',
                                                                        }}
                                                                        title='Pick a color'
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        ...
                                        formData.features.forEach((feature, index) => {
                                            // Include feature ID if it exists (for existing features from DB)
                                            if (feature.id && typeof feature.id === 'number') {
                                                payload.append(`features[${index}][id]`, String(feature.id))
                                            }
                                        ...
                                        formData.technical_specs.forEach((spec, index) => {
                                            // Include spec ID if it exists (for existing specs from DB)
                                            if (spec.id && typeof spec.id === 'number') {
                                                payload.append(`technical_specs[${index}][id]`, String(spec.id))
                                            }
                                        ...
                                        // Primary image ID
                                        if (primaryImageId) {
                                            payload.append('primary_image_id', primaryImageId)
                                        }

...
export default function Navbar() {
    ...
    const [showScrolledShape, setShowScrolledShape] = useState(false);
    ...
    useEffect(() => {
        let timer;
        if (isScrolled) {
            timer = setTimeout(() => setShowScrolledShape(true), 0);
        } else {
            setShowScrolledShape(false);
        }
    ...
    return (
        <>
            <nav
                className={`fixed top-11 left-0 right-0 z-50 transition-all duration-500 ${showBackground ? "bg-[#161719]/95 backdrop-blur-md shadow-2xl" : "bg-transparent"
                    }`}
                onMouseLeave={() => {
                    setDesktopDropdownOpen(null);
                    setHoveredItem(null);
            ...
            >
                <div className="mx-auto px-3 lg:px-8 h-24 flex items-center">
                    <div className="hidden xl:block w-[240px]" aria-hidden="true" />

                    <div className="hidden xl:flex flex-1 justify-center">
                        <div className="flex items-center gap-10">
                            <div className="relative flex items-center px-1 py-1">
                                <div
                                    className={`pointer-events-none absolute left-1/2 -top-[1.7rem] z-0 h-[38px] w-[211px] -translate-x-1/2 transition-opacity duration-500 ease-in-out ${
                                        showScrolledShape ? "opacity-100" : "opacity-0"
                                    }`}
                                >
                                    <img
                                        src="/assets/images/logo-shape.png"
                                        alt=""
                                        aria-hidden="true"
                                ...
                                <Link href="/" className="relative z-10 flex items-center">
                                    <img
                                        ...
                                        alt="Invention Training"
                                        className="-mt-[1.2rem] h-[7.75rem] w-auto rounded-t-[1.05rem] object-contain"
                                    />
                                </Link>
                            </div>
                            {navItems.map((item) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <span
                                                className={`absolute left-0 -bottom-[6px] h-px w-full origin-left bg-[#d7b55a] transition-transform duration-300 ${hoveredItem === item.label || isActiveItem || isDropdownOpen ? "scale-x-100" : "scale-x-0"
                                                    }`}
                                            />
                                        </Link>
                                                    ...
                                                    {item.children.map((child) => {
                                                        ...
                                                        return (
                                                            <Link
                                                                key={child.path}
                                                                href={child.path}
                                                                className={`block rounded-xl px-4 py-3 text-sm tracking-[0.06em] uppercase transition-colors ${childActive ? "bg-white/10 text-[#f7e4af]" : "text-white/75 hover:bg-white/[0.06] hover:text-white"
                                                                    }`}
                                                            >
                                                                {child.label}
                                                            </Link>
                    ...
                    <div className="flex w-full items-center justify-between xl:ml-auto xl:w-[240px] xl:justify-end">
                        <Link href="/" className="xl:hidden flex items-center">
                            <img src="/assets/images/ITC-logo.jpg" alt="Invention Training" className="h-20 w-auto object-contain" />
                        </Link>
                ...
                {isMobileMenuOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed top-0 right-0 bottom-0 z-[70] w-full max-w-md bg-[#161719] p-6 overflow-y-auto"
                        >
                                    ...
                                    return (
                                        <div key={item.label} className="border-b border-white/10">
                                            <div className="flex items-center justify-between py-[1.125rem]">
                                                <Link
                                                    href={item.path}
                                                    className={`text-[1.35rem] font-light tracking-[0.06em] leading-none ${isActiveItem ? "text-[#f7e4af]" : "text-white"
                                                        }`}
                                                    onClick={closeMobileMenu}
                                                >
                                                    {item.label}

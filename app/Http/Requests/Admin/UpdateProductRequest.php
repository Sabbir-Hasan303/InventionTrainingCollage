...
class UpdateProductRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            // Validate image files (including AVIF support)
            // if ($this->hasFile('images')) {
            //     $allowedMimes = ['jpeg', 'png', 'gif', 'bmp', 'webp', 'svg+xml', 'avif'];
            //     foreach ($this->file('images') as $file) {
            //         $mimeType = $file->getMimeType();
            //         $extension = $file->getClientOriginalExtension();
            //         // dd($mimeType, $extension);
            //         // Check if mime type or extension is allowed
            //         if (!in_array($mimeType, array_map(fn($m) => "image/$m", $allowedMimes)) &&
            //             !in_array(strtolower($extension), $allowedMimes)) {
            //             $validator->errors()->add('images', "File '{$file->getClientOriginalName()}' must be an image (JPEG, PNG, GIF, BMP, WebP, SVG, or AVIF).");
            //             break;
            //         }
            //     }
            // }
